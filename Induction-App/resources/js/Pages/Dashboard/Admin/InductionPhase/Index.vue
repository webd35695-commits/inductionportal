    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">
          Induction Phase Management
        </h2>
        <Link
          :href="route('InductionPhase.create')"
        ...
        >
          <svg
            class="w-5 h-5 mr-2 text-white group-hover:text-white"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      />
      <SuccessPopup
        v-if="showDeleteSuccess && $page.props.flash.message"
      ...
      <div class="overflow-hidden bg-white shadow sm:rounded-lg">
        <div class="p-6 border-b border-gray-200">
          <table
            id="inductionPhaseTable"
            class="w-full display responsive nowrap"
          >
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                >
                  <span
                    :class="{
                      'bg-green-100 text-green-800': phase.status === 'Active',
                      'bg-red-100 text-red-800': phase.status === 'Inactive',
                    }"
                ...
                <td
                  class="px-6 py-4 text-sm font-medium border-b border-gray-200 whitespace-nowrap"
                >
                  <div class="flex space-x-3">

                    <Link
                      :href="route('InductionPhase.edit', phase.id)"
                    ...
                    <button
                      @click="deleteInductionPhase(phase.id)"
...
const deleteInductionPhase = (id) => {
  if (confirm("Are you sure you want to delete this Induction Phase?")) {
    router.delete(route("InductionPhase.destroy", id), {
      preserveScroll: true,
      onSuccess: () => {
  ...
  $("#inductionPhaseTable").DataTable({
    ...
    initComplete: function () {
      $(".dataTables_filter input").addClass(
        "block w-full px-3 py-2 text-base border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
      );
    ...
    drawCallback: function () {
      $(".paginate_button").addClass(
        "px-3 py-1 rounded-md border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50"
      );
      $(".paginate_button.current").addClass(
        "bg-indigo-50 border-indigo-500 text-indigo-600 hover:bg-indigo-50"
      );
