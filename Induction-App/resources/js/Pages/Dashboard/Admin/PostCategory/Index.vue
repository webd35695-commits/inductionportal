    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Post Category Management</h2>
                <Link
                    :href="route('PostCategory.create')"
            ...
            <div class="overflow-hidden bg-white shadow sm:rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <table id="postCategoryTable" class="w-full display responsive nowrap">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase border-b border-gray-200">Category Name</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="postCategory in postCategories" :key="postCategory.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 border-b border-gray-200 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="font-medium text-gray-900">{{ postCategory.name }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 border-b border-gray-200 whitespace-nowrap">
                                    <span
                                        :class="{
                                            'bg-green-100 text-green-800': postCategory.status === 'Active',
                                            'bg-red-100 text-red-800': postCategory.status === 'Inactive',
                                        }"
                                ...
                                <td class="px-6 py-4 text-sm font-medium border-b border-gray-200 whitespace-nowrap">
                                    <div class="flex space-x-3">
                                        <Link
                                            :href="route('PostCategory.show', postCategory.id)"
                                        ...
                                        <Link
                                            :href="route('PostCategory.edit', postCategory.id)"
                                            class="p-1 text-green-600 rounded-md hover:text-green-900 hover:bg-green-50"
                                            title="Edit"
                                        >
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </Link>
                                        <button
                                            @click="deletePostCategory(postCategory.id)"
...
const deletePostCategory = (id) => {
    if (confirm('Are you sure you want to delete this post category?')) {
        router.delete(route('PostCategory.destroy', id), {
            preserveScroll: true,
            onSuccess: () => {
