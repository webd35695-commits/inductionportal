  ...
  >
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-gray-900">Edit Post Level</h3>
        <button
          @click="closeModal"
...
const {postLevel,isOpen}= defineProps({
  isOpen: {
    type: Boolean,
    required: true,
  },
...
watch(
  ...
  (newPostLevel) => {
    if (newPostLevel) {
      form.name = newPostLevel.name || '';
      form.status = newPostLevel.status || '';
    }
  },
  { immediate: true }
);
...
const submitForm = () => {
  form.put(route('post_levels.update', postLevel.id), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset();
      emit('close');
